// features/boards/components/Modals/DealDetailModal.tsx
...
// ✅ Passo B: Supabase client (ajuste o path se o seu projeto usa outro)
...
/**
 * ✅ Passo B: Tipos do schema dinâmico por estágio
 * (a tabela stage_form_schemas tem uma coluna "schema" jsonb)
 */
type StageSchemaField = {
  key: string;
  ...
  required_if?: { field: string; equals: string };
};

type StageFormSchema = {
  title?: string;
  save_root: string;
  fields: StageSchemaField[];
};
...
export const DealDetailModal: React.FC<DealDetailModalProps> = ({ dealId, isOpen, onClose }) => {
  ...
  /**
   * ✅ Passo B: carregar schema do formulário do estágio atual
   */
  const [stageSchema, setStageSchema] = useState<StageFormSchema | null>(null);
  ...
  const inner = (
    <>
      <div
        data-stage-save-root={stageSchema?.save_root || ''} // ✅ usa stageSchema (evita lint de unused)
        className={
          isMobile
          ...
          {dealBoard ? (
            <StageProgressBar
              ...
              onStageClick={stageId => {
                ...
                const isLostStage =
                  dealBoard.lostStageId === stageId || targetStage?.linkedLifecycleStage === 'OTHER';
            ...
            <div className="h-14 border-b border-slate-200 dark:border-white/5 flex items-center px-6 shrink-0">
              <div className="flex gap-6">
                <button
                  ...
                  className={`text-sm font-bold h-14 border-b-2 transition-colors ${
                    activeTab === 'timeline'
